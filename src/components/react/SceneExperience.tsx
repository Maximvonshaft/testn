...
import { materials, systems, type MaterialId, type SystemId, type SystemVisual } from '@/data/catalog';
...
function preloadSystemAtlas(system: SystemVisual): Promise<void> {
  ...
  const promise = (async () => {
    ...
    image.src = url;
    if (!image.complete) await new Promise<void>((resolve, reject) => { image.onload = () => resolve(); image.onerror = () => reject(new Error(`Unable to load scene atlas: ${url}`)); });
    if (typeof image.decode === 'function') await image.decode();
  })().catch((error) => { atlasCache.delete(url); throw error; });
...
function SceneLayer({ system, materialIndex, priority, reduceMotion }: { system: SystemVisual; materialIndex: number; priority: boolean; reduceMotion: boolean }) {
  ...
  return <motion.div className={styles.sceneLayer} initial={{ opacity: 0, scale: 1.012 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: .996 }} transition={{ duration: reduceMotion ? 0 : .58, ease: [.22, 1, .36, 1] }}>
    <picture className={styles.atlasPicture}>
      <source media="(max-width: 760px)" srcSet={system.mobileAtlas} type="image/avif" />
      <img className={styles.atlasImage} data-state-atlas src={system.desktopAtlas} alt="" aria-hidden="true" width="4500" height="312" decoding="async" fetchPriority={priority ? 'high' : 'auto'} style={{ transform: `translate3d(${translateX},0,0)` }} />
    </picture>
  </motion.div>;
}
...
export default function SceneExperience({ copy }: Props) {
  ...
  useEffect(() => { window.dispatchEvent(new CustomEvent('aquastone:selection', { detail: { system: systemId, material: material.name } })); }, [systemId, material.name]);
  useEffect(() => {
  ...
  const changeSystem = async (id: SystemId) => {
    ...
    setRequestedSystemId(id); setLoading(true);
    try { await preloadSystemAtlas(target); if (requestSequence.current === sequence) setSystemId(id); }
    catch { if (requestSequence.current === sequence) setRequestedSystemId(systemId); }
    finally { if (requestSequence.current === sequence) setLoading(false); }
  };

  return <section className={styles.root} aria-label={copy.hero.railTitle} data-scene-experience>
    <div className={styles.stageShell}>
      <aside className={styles.rail}>
        ...
        <div className={styles.systemList} role="tablist" aria-orientation="vertical">
          {systems.map((item) => <button key={item.id} type="button" className={styles.systemButton} data-active={item.id === systemId} data-pending={loading && item.id === requestedSystemId} role="tab" aria-selected={item.id === systemId} aria-busy={loading && item.id === requestedSystemId} onClick={() => void changeSystem(item.id)}><SystemIcon id={item.id}/><span className={styles.systemLabel}>{copy.systems[item.id].label}</span><span className={styles.chevron}>›</span></button>)}
        </div>
      ...
      <div className={styles.hero} data-loading={loading} data-scene-state={`${systemId}:${materialId}`} data-scene-system={systemId} data-scene-material={materialId} aria-busy={loading} aria-label={`${text.label}: ${material.name}`}>
        <AnimatePresence mode="sync" initial={false}><SceneLayer key={`${systemId}-${materialId}`} system={system} materialIndex={material.atlasIndex} priority={systemId === 'bathroom'} reduceMotion={Boolean(reduceMotion)} /></AnimatePresence>
        <div className={styles.heroShade}/><div className={styles.loadingVeil} aria-hidden="true"><span/></div>
        ...
        <AnimatePresence mode="wait" initial={false}><motion.div key={systemId} className={styles.copyBlock} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: reduceMotion ? 0 : .45 }}><p className={styles.heroEyebrow}>{text.eyebrow}</p><h1 className={styles.heroTitle}>{text.title}</h1><p className={styles.heroDescription}>{text.description}</p><a className={styles.heroLink} href="#technology">{copy.actions.details} &nbsp;→</a></motion.div></AnimatePresence>
    ...
    <div className={styles.mobileSystemStrip} role="tablist" aria-label={copy.hero.railTitle}>{systems.map((item) => <button key={item.id} type="button" className={styles.mobileSystemButton} data-active={item.id === systemId} data-pending={loading && item.id === requestedSystemId} role="tab" aria-selected={item.id === systemId} onClick={() => void changeSystem(item.id)}><SystemIcon id={item.id}/><span>{copy.systems[item.id].label}</span></button>)}</div>
    <div className={styles.materialDock}><div className={styles.materialDockHeader}><span>{copy.hero.materialTitle}</span><span aria-live="polite">{copy.hero.selected}: {material.name}</span></div><div className={styles.materialTrack} role="listbox" aria-label={copy.hero.materialTitle}>{materials.map((item) => <button key={item.id} type="button" className={styles.materialButton} data-active={item.id === materialId} role="option" aria-selected={item.id === materialId} onClick={() => setMaterialId(item.id)}><img className={styles.slab} src={item.image} alt="" width="54" height="80" loading="lazy"/><span className={styles.materialName}>{item.name}</span></button>)}</div></div>
    <div className={styles.benefits} role="region" tabIndex={0} aria-label="System benefits">{copy.benefits.map((benefit, index) => <div className={styles.benefit} key={benefit.title}><svg className={styles.benefitIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">{benefitIcons[index]}</svg><span><strong className={styles.benefitTitle}>{benefit.title}</strong><small className={styles.benefitDetail}>{benefit.detail}</small></span></div>)}</div>
  </section>;
}
