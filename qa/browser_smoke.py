...
def exercise(page: Page, base_url: str, label: str) -> dict[str, object]:
    ...
    # Make visual tests deterministic and prove the local hero fallback works.
    page.route(re.compile(r"^https://images\.(?:unsplash|pexels)\.com/"), lambda route: route.abort())
    ...
    return {"label": label, "status": "passed", "screenshot": screenshot.name}
